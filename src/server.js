...
class Server {
  ...
  setupRoutes() {
    ...
    this.app.get('/health', async (req, res) => {
      try {
        const [dbHealth, redisHealth] = await Promise.all([
          db.raw('SELECT 1').then(() => ({ status: 'healthy' })).catch(err => ({ status: 'unhealthy', error: err.message })),
    ...
    if (process.env.NODE_ENV !== 'production') {
      const swaggerOptions = {
        definition: {
          ...
          info: {
            ...
            description: process.env.SWAGGER_DESCRIPTION || 'A comprehensive Todo application API',
            contact: {
              name: 'API Support',
      ...
      logger.info(`API documentation available at http://${this.host}:${this.port}/api-docs`);
    }
  ...
  async setupGraphQL() {
    ...
    this.apolloServer = new ApolloServer({
      ...
      formatError: (error) => {
        ...
        if (process.env.NODE_ENV === 'production' && !error.message.startsWith('GraphQL error:')) {
          ...
          requestDidStart() {
            return {
              async didResolveOperation({ request, context }) {
                ...
                const requestCount = await redisManager.get(rateLimitKey) || 0;
    ...
    logger.info(`GraphQL playground available at http://${this.host}:${this.port}${this.apolloServer.graphqlPath}`);
  }
